...
rppicomidi::Mc_settings_file::Mc_settings_file(Mc_bridge_model& model_) : model{model_}, vid{0}, pid{0}
{
...
void rppicomidi::Mc_settings_file::set_vid_pid(uint16_t vid_, uint16_t pid_)
{
    vid = vid_;
    pid = pid_;
}
...
int rppicomidi::Mc_settings_file::load_settings_string(char** raw_settings_ptr)
{
    if (vid != 0 && pid != 0) {
        int error_code = pico_mount(false);
        if (error_code != 0) {
...
bool rppicomidi::Mc_settings_file::load()
{
